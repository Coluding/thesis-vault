...
> **Reframe (2026-06-17).** The premise below — "the shortcut adapter looks
> step-count robust qualitatively" — no longer holds on real data. The first
> real-backbone run (AVID/DynamiCrafter, `anchor_prob=0.45`, larger MetaWorld)
> **degraded** with a **volatile `shortcut_direction_loss`** while base_loss
> stayed stable; the earlier robustness reads as small-data overfit. See
> [[../../30_Knowledge/experiments/avid-shortcut-anchor045-volatile-loss]].
> The control is still meaningful, but the question shifts: not "does shortcut
> training *cause* the robustness" but **"does the shortcut term help at all
> (vs anchor-only) once we're not overfitting, and is the loss volatility real
> instability or a step-size-mixing logging artifact?"** Don't run this clean
> A/B until the per-step-size loss diagnosis (in progress) clarifies whether
> the shortcut objective is even stable. Blocked-ish on that.
