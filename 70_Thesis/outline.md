...
## 2. Related work — `chapters/20-related-work.tex` · **5–6 pp**

_Function: make our question the obvious next one. **Four sections**, per
the structure Lukas set 2026-08-08._

- 2.1 **Diffusion and flow matching** — **draft-complete** — the base. Both families as *transport*, using [[../30_Knowledge/related-work/…]]'s SDE view so flow matching is the straight-trajectory case rather than a separate object. Fixes notation, the three prediction targets, trajectory curvature, step size
- 2.2 **World models** — **draft-complete** (2026-08-08) — sorted by **how much of the generative model is trained**: from-scratch latent (Dreamer line) · from-scratch generative (DIAMOND, Genie) · pretrained then fully fine-tuned (GameNGen, UniSim) · **frozen plus a trained module (AVID)**. Ends on why the last two differ and what we give up
- 2.3 **Adapting a pretrained generative model** — **draft-complete** — two orthogonal axes. §2.3.1 **what is changed**: *all parameters* (fine-tuning, LoRA, bottleneck adapters) · *hidden states* (ControlNet, T2I-Adapter, IP-Adapter) · *output only* (AVID). §2.3.2 **how the conditioning enters**: FiLM/adaLN vs cross-attention, and the DiT-vs-GenTron disagreement resolved by signal type
- 2.4 **Reducing the number of sampling steps** — **draft-complete** — better solvers (DDIM, DPM-Solver, training-free floor) · distilling into a new model (progressive, consistency, DMD, adversarial) · **conditioning on the step size** (shortcut, HOMO, MeanFlow) · and *where the acceleration lives* (LCM-LoRA, SCFM, PDD), which is our group

⚠ **The selection argument is NOT here.** Ch2 presents the options; Method
§3.2 derives why we take the output-only route. Deliberate split.
...
- 3.2 **Adapter families and the selection** — **draft-complete** (expanded 2026-08-08, ~2.5 pp) — *(was a standalone chapter; merged for the page limit.)* Now built on a **four-rung access ladder** (`tab:access-ladder`): writable weights → differentiable forward → readable activations → **callable denoiser only (us)**, with rung 5 (finished samples only) explicitly **empty for every method including ours**. That concession is load-bearing: "works with closed API models" is false as usually stated, since every adapter needs the denoiser at *intermediate* states. Then the cost criterion with the 14,175-token/93 GB memory argument and the measured 3.1×, plus a **new §"Why not fine-tune the base instead?"** answering the [[../30_Knowledge/related-work/vid2world]] objection (access · economy · AVID's functional-split finding) and conceding the missing frozen-vs-finetuned measurement to Limitations. Keeps: why four families are implemented; the `d`-dependent-weight-update-is-a-hypernetwork tension. ⚠ **Rung-4 claim verified in code**: base forward under `no_grad` (`models/adapted_model.py:180`), and the Wan adapter's AdaLN/cross-attention sit in the adapter's *own* tiny DiT, not in the base. src: [[../30_Knowledge/theory/interior-vs-output-adapters-backward-cost]], [[../30_Knowledge/theory/unicon-output-adapters-detached-backward]], [[../30_Knowledge/experiments/20260803-lora-costs-3x-the-walltime-of-an-output-adapter]]
...
- 5.3 **The injection pathway decides (D2)** — **draft-complete** (2026-08-07, ~2.5 pp) — the failure · four alternatives excluded by measurement (data, capacity, family/EA, scale-and-alignment) · the depth localisation · the matched-contribution A/B with the temporal control · **scope** (low-dimensional spatially-global signal; frozen base) and an explicit what-is-ours-and-what-is-not paragraph. src: [[../30_Knowledge/writing/evidence-map]], [[../30_Knowledge/related-work/pathway-claim-prior-art-verdict]]
