...
- 2.2 **Adapting frozen generative models** — **draft-complete** (2026-08-07, ~0.9 pp) — ordered by **how much of the base each method needs**: inside the weights (Houlsby, LoRA) → inside the activations (ControlNet, T2I-Adapter, IP-Adapter) → outside entirely (AVID). Closes on the **zero-init convention** as an unexamined tension, which sets up §5.4. ⚠ **Presents the options only; the *selection* argument lives in Method §3.2** (no need for base params, hidden activations or gradients through the base). Notes `lora.md`/`controlnet.md` still to be written, but metadata is verified and in `refs.bib`
